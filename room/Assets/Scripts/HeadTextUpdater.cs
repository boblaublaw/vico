...
public class HeadTextUpdater : MonoBehaviour 
{
	...
	void Update () 
	{
		if (NuitrackManager.CurrentUser != 0)
		{
			...
			debugText.text = string.Format(userInfoText, headJoint.Real.X, headJoint.Real.Y, headJoint.Real.Z, eulerAngles.x, eulerAngles.y, eulerAngles.z);
		}
		else
		{
			debugText.text = textNoUser;
		}
	}
}
